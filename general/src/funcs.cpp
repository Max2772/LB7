...
int readCharToStringForChoice(char* input){  
    ...
    for(int i = 0; i < 80; ++i){
        input[i] = getchar();
        if(input[i] != '1' && input[i] != '2' && input[i] != '3' && input[i] != '4' && input[i] != '5' && input[i] != '\n'){
            invalidInput = true;
        }
        if(input[i] == '\n'){
            tooLongInput = false;
    ...
    if(tooLongInput){
        cout << "Ввод слишком длинный, введите только 1,2,3,4 или 5: ";
        while((getchar()) != '\n');
        return -INF;
    }

    if(invalidInput || (input[1] != '\n')){
        cout << "Некорректный ввод, введите только 1,2,3,4 или 5: ";
        return -INF;
    }else if(input[0] == '\n')
        return -INF;
